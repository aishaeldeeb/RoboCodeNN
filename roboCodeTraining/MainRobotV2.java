...
public class MainRobotV2 extends AdvancedRobot {
    //look up table object


    public static LookUpTableV2 lut = new LookUpTableV2();

    //stats
    public static int rounds;
    public static int totalRounds;
    public static final int avgRounds = 10; //number of round to average the win rate
...
//    private static double enemyTime;
    ...
    public static int quantizedEnemyBearing;
    public static int quantizedHeading;
...
//            double ene = getBearingRadians();
//            int stateIndex = lut.quantizeStates(enemyDistance, enemyBearing, heading, xPos, yPos);
//            int stateIndex = lut.quantizeStates(enemyDistance, enemyBearing, heading, xPos, yPos);

            //quantize

            //TODO: use cos and sin of heading >> cos(angle) then qunatize (-1,1)
            //TODO: discard the bearing because it has zero values for some reason
            //TODO: debug updating LUT with zeros
            int quantizedEnemyDistance = lut.quantizeEnemyDistance(enemyDistance);
            int quantizedEnemyBearing = lut.quantizeEnemyBearing(enemyBearing);
            int quantizedHeading = lut.quantizeEnemyBearing(heading);

            int quantizedXPos = lut.quantizeXPos(xPos);
            int quantizedYPos = lut.quantizeXPos(yPos);

            //pick action index based on state index
            int actionIndex = lut.getAction(quantizedHeading, quantizedEnemyDistance, quantizedEnemyBearing, quantizedXPos, quantizedYPos); //TODO: random number always 4
            //update look up table
            lut.updateLUT(onPolicy, rewards, quantizedHeading, quantizedEnemyDistance, quantizedEnemyBearing, quantizedXPos, quantizedYPos, actionIndex);
